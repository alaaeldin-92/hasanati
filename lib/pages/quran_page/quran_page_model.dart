...
import 'quran_page_widget.dart' show QuranPageWidget;
...
class QuranPageModel extends FlutterFlowModel<QuranPageWidget> {
  ...
  Future updateMemorized(BuildContext context) async {
    ApiCallResponse? quranPageJSON;

    quranPageJSON = await QuranFontImlaeiPageCall.call(
      page: page,
    );
    if (functions.checkVerseKeysExist(
        getJsonField(
          (quranPageJSON?.jsonBody ?? ''),
          r'''$.verses''',
          true,
        )!,
        FFAppState().quranVersesMemorized.toList())) {
      memorized = true;
    } else {
      memorized = false;
    }
