...
import '/custom_code/actions/change_status_bar_color.dart' as actions;
...
void main() async {
  ...
  await actions.changeStatusBarColor(
      FlutterFlowTheme.of(appState as BuildContext).primaryBackground);
}
  ...
  void checkAndUpdateDB() async {
    ...
    if (!FFAppState().quranVersesMemorizedAddSession.isEmpty) {
      ...
      await quranVersesMemorizedAddSession.docs.first.reference.update({
        'verse':
            FieldValue.arrayUnion(FFAppState().quranVersesMemorizedAddSession),
        // Add other fields you want to update
      });
    ...
    if (!FFAppState().quranVersesFavoriteAddSession.isEmpty) {
      ...
      await quranVersesFavoriteAddSession.docs.first.reference.update({
        'verse':
            FieldValue.arrayUnion(FFAppState().quranVersesFavoriteAddSession),
        // Add other fields you want to update
      });
